...
def filter_by_year(statistics, year, yearid):
    ...
    """
    # It directly returns the year accordignly
    return [dictionaries for dictionaries in statistics if year == int(dictionaries[yearid])]
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    player_id_stat_list = list()
    # Loop through the the statisitcs list for dictionaries
    for stat_dict in statistics:
        player_id_stat = (stat_dict[info["playerid"]],
                          formula(info, stat_dict))
        player_id_stat_list.append(player_id_stat)
    # Sort the list of tuples and finally return it
    list_of_tuples = sorted(player_id_stat_list,
                            key=lambda result: result[1], reverse=True)[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    the_list = []
    # Get info and store it in statistics list
    statistics_list = read_csv_as_list_dict(
        info['masterfile'], info['separator'], info['quote'])
    # Loops through top_ids_and_stats
    for player_id_stat in top_ids_and_stats:
        player_id = player_id_stat[0]
        for dict_ in statistics_list:
            if dict_[info['playerid']] == player_id:
                first_name = dict_[info['firstname']]
                last_name = dict_[info['lastname']]
                stat = player_id_stat[1]
                # Format the string
                # print("{:.3f}".format(stat))
                stat = '{:.3f}'.format(stat)
                format_string = stat + ' --- ' + first_name + ' ' + last_name
                the_list.append(format_string)
    return the_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    # Read csv file and save to statsitcs list
    statistics_list = read_csv_as_list_dict(
        info['battingfile'], info['separator'], info['quote'])

    filtered_dict = filter_by_year(statistics_list, year, info['yearid'])
    # Top player ids along with compound statistic from filtered dict
    tuples_from_top_players = top_player_ids(
        info, filtered_dict, formula, numplayers)
    return lookup_player_names(info, tuples_from_top_players)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    """
    player_id_agg_dict = dict()
    # Loop through statistics list of dictionaries and check for player id
    for row in statistics:
        if row[playerid] not in player_id_agg_dict:
            player_id_agg_dict[row[playerid]] = {}
            player_id_agg_dict[row[playerid]][playerid] = row[playerid]
            for item in fields:
                player_id_agg_dict[row[playerid]][item] = int(row[item])
        else:
            for item in fields:
                player_id_agg_dict[row[playerid]][item] += int(row[item])
    return player_id_agg_dict
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    statistics_list = read_csv_as_list_dict(
        info['battingfile'], info['separator'], info['quote'])
    # Get aggregated data dictionary from aggregate_by_player_id
    aggregate_statistics = aggregate_by_player_id(statistics_list,
                                                  info['playerid'], info['battingfields'])
    # Converting aggregate_statistics to a list of dictionaries
    list_of_dicts = [value for key, value in aggregate_statistics.items()]
    # Get top numplayers from aggregate_statistics list of dict
    get_top_players = top_player_ids(info, list_of_dicts, formula, numplayers)
    # Get no_of_top_players from lookup player names
    no_of_top_players = lookup_player_names(info, get_top_players)

    return no_of_top_players
##
