...
def build_plot_values(gdpinfo, gdpdata):
    ...
    gdp_tuples_list = []
    for data in gdpdata.items():
      # Check if fields have values. If there is an error/no value
      # continue the iteration
        try:
            if int(data[0]) in range(gdpinfo['min_year'], gdpinfo['max_year']+1):
                gdp_tuples_list.append((int(data[0]), float(data[1])))
        except ValueError:
            pass
    # It should be sorted accordignly
    gdp_tuples_list.sort(key=lambda result: result[0], reverse=False)
    return gdp_tuples_list
...
def build_plot_dict(gdpinfo, country_list):
    ...
    for country in country_list:
        if country in nested_dict_data.keys():
            row = list(nested_dict_data[country].items())
            country_dict = dict(row)
            plot[country] = build_plot_values(gdpinfo, country_dict)
        else:
            plot[country] = []
    return plot
